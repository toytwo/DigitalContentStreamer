...
export default function ContentEditPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([getContent(contentId), getSubscriptionTiers()])
      .then(([content, tiers]) => {
        if (isMounted) {
          if (content) {
            const c = content as Record<string, unknown>;
            setTitle((c.title as string) || "");
            setContentType((c.type as string) || CONTENT_TYPE_OPTIONS[0].value);
            setReleaseDate((c.release_date as string) || "");
            setRequiredTier(((c.required_tier as number) || 1).toString());
            setGenre((c.genre as string) || GENRE_OPTIONS[0].value);
            setRuntimeMinutes(((c.runtime_minutes as number) || 0).toString());
            setOriginalLanguage((c.original_language as string) || LANGUAGE_OPTIONS[0].value);
            setAgeRating((c.age_rating as string) || AGE_RATING_OPTIONS[0].value);
          } else {
      ...
      .catch((error) => {
        console.error("Failed to load content or tiers:", error);
        if (isMounted) {
  ...
  }, [sessionUser, isCheckingSession, contentId]);
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const request: Partial<ContentRequest> = {
        title,
        type: contentType,
      ...
      setErrorMessage(error instanceof Error ? error.message : "Failed to update content");
    } finally {
      setIsSubmitting(false);
    }
  }

  if (isCheckingSession || isLoadingContent) {
    return (
      ...
      <PageCard className="max-w-4xl">
        <div className="mb-8 space-y-2 text-center">
          <PageHeader
            ...
            title="Edit content"
            description="Update the content item details below."
            align="center"
          />
        </div>
